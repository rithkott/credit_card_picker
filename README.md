...
Ranks every 1–`max_cards` portfolio of cards you can get approved for by net annual value (`--json` for machine output, `--as-of` for reproducible runs; points priced at each program's engaged-average cpp — see docs/plans/08-simplified-valuation.md). Deterministic by design: identical inputs produce byte-identical output, and every valuation assumption is echoed in the run header. The optimizer runtime stays pure stdlib + pyyaml (`numpy`, if installed, vectorizes the search bounds — same results either way); `scipy` is used only by the test suite as an LP oracle (`pip install scipy` to run `tests/test_assign_exact.py`). Design specs: [docs/plans/02-optimizer.md](docs/plans/02-optimizer.md), [docs/plans/10-optimizer-overhaul.md](docs/plans/10-optimizer-overhaul.md).
