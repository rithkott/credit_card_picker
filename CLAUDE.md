...
- `data/business/schema/business-card.schema.json`, `data/business/meta/` registries, and `scripts/validate_business_cards.py` (plan 22 — the business dataset plane has the same diagram obligations as the consumer one)
...
- **Business dataset (plan 22):** `data/business/` is the parallel corpus for the business product (business.cardsharp.dev, in progress — see `docs/plans/22-business-cards-build-design.md`). All consumer curation rules apply identically (hand-curated, sources + verification, no scraping, no monetization); business card files must pass `python3 scripts/validate_business_cards.py`; the backlog is `docs/business-card-backlog.md`; cross-card assumptions live in `data/business/meta/` (registries are COPIED from the consumer side where they overlap — never shared — and the business validator warns on point-valuation drift). The business product has NO statement parsing: spend is entered manually. `tools/card-entry-form.html` stays consumer-scoped (no business form exists yet).
...
- `server/business_api.py` wraps `scripts/optimize_business.py` in-process (plan 22D, mounted by `app.py` under `/api/business/*`) — any change to `parse_business_profile`'s contract, the business `run()` bundle shape, any `/api/business/*` response shape, or the business engine's `TIER_ORDER` / `USER_DEFAULTS` / `COMPANY_DEFAULTS` / `PERSONAL_DEFAULTS` / `PERSONAL_GATEWAYS` / `REWARD_KINDS` constants must update `tests/test_business_server_api.py` + `site/src/business/types.ts` in the same change. Consumer endpoints must stay byte-identical (pinned by `tests/test_server_api.py`).
